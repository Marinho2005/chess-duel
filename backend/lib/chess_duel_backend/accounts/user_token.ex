defmodule ChessDuelBackend.Accounts.UserToken do
  use Ecto.Schema
  import Ecto.Query
  alias ChessDuelBackend.Accounts.UserToken
  ...
  def build_api_token(user) do
    ...
    authenticated_at = user.authenticated_at || NaiveDateTime.utc_now(:second)
  ...
  def api_token_query(encoded_token) do
    with {:ok, token} <- Base.url_decode64(encoded_token, padding: false) do
      hashed_token = :crypto.hash(@hash_algorithm, token)
      {:ok, by_token_and_context_query(hashed_token, "api")}
    else
      :error -> :error
    end
  end
