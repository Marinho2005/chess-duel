...
- [~] **6.3 Espectadores/streaming** — Broadcasts do Lichess aparecem somente como previews estáticos no lobby e nas páginas de observação, atualizados periodicamente via REST. Cada card abre a URL da partida específica no Lichess em nova aba, com indicação de link externo. `/observar` mantém o catálogo público completo de torneios e até 30 previews por torneio no feed; `/observar/torneio/:tournamentId` mantém metadados, seleção de rodadas e previews das partidas. Cache e polling continuam ativos para os previews, obtidos diretamente do JSON de cada rodada; a exportação e a interpretação de PGN não são necessárias para exibir tabuleiros. A espectação interna de broadcasts, seu Channel e as avaliações Stockfish exclusivas foram descontinuados. Os previews de partidas ChessDuel e seus filtros permanecem; espectação completa das partidas internas continua pendente.
...
- `GET /api/broadcasts/rounds/:round_id/games` retorna as partidas da rodada, incluindo encerradas, com FEN, jogadores e `lichess_url` específica para os previews. Posição, jogadores e último lance vêm do JSON da rodada, sem requisição de PGN. O cache sob demanda mantém metadados do torneio por cinco minutos e posições de rodadas em andamento por 20 segundos.
...
- `/observar` oferece as abas Torneios e ChessDuel; `/observar/torneio/:tournamentId` abre as partidas agrupadas. Os cards de broadcast do lobby e do torneio abrem `lichess_url` em nova aba (`target="_blank"`, `rel="noopener noreferrer"`), com a ação “Ver no Lichess”.
