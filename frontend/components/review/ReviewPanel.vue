...
<template>
  <aside class="analysis-panel">
    ...
    <section class="players" aria-label="Jogadores da partida">
      <b><i class="white-piece" />{{ whitePlayer?.nickname || 'Brancas' }}</b>
      <span>contra</span>
      <b><i class="black-piece" />{{ blackPlayer?.nickname || 'Pretas' }}</b>
    </section>
...
<style scoped>
.analysis-panel { display: flex; min-height: 0; flex-direction: column; overflow: hidden; color: #3c2b20; background: #fffaf0f2; border: 1px solid #dccbb2; border-radius: 12px; box-shadow: 0 16px 38px #60401f1b; }
...
nav { display: grid; padding: .65rem; grid-template-columns: repeat(2, 1fr) 1.25fr repeat(2, 1fr); gap: .35rem; background: #efe3d1; border-top: 1px solid #dccbb2; }nav button { display: grid; min-height: 40px; place-items: center; color: #68462f; background: #fffaf0; border: 1px solid #d8c6ad; border-radius: 7px; cursor: pointer; }nav button:hover:not(:disabled) { color: white; background: #925b35; border-color: #925b35; }nav button:disabled { opacity: .35; cursor: default; }nav > span { display: grid; place-items: center; color: #806d5d; font-size: .72rem; }
</style>
