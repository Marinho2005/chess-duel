...
<style scoped>
.move-table { min-height: 0; overflow: hidden; border: 1px solid #eadfce; border-radius: 10px; background: #fffaf0; }.table-head,.move-table li { display: grid; grid-template-columns: 38px 1fr 1fr; align-items: center; }.table-head { padding: .5rem .55rem; color: #876f5d; background: #eee2d0; font-size: .68rem; letter-spacing: .03em; }.table-head b { padding-left: .65rem; }.move-table ol { max-height: 330px; margin: 0; padding: 0; overflow-y: auto; list-style: none; }.move-table li { min-height: 38px; padding: 0 .55rem; }.move-table li:nth-child(even) { background: #7d553308; }.move-table li > span { color: #9a816e; font-size: .72rem; text-align: right; }.move-table button { min-width: 0; padding: .55rem .65rem; overflow: hidden; color: #3c2b20; text-align: left; background: transparent; border: 0; border-radius: 6px; font: 700 .86rem var(--font-mono); text-overflow: ellipsis; white-space: nowrap; cursor: pointer; }.move-table button:hover { background: #eadcc7; }.move-table button.active { color: #4b3829; background: #eadcc7; box-shadow: inset 3px 0 #c18a3a; }.move-table button:focus-visible { outline: 2px solid #d0a45d; outline-offset: 1px; }
.move-table { color:var(--text); background:var(--surface); border-color:var(--border); }.table-head { color:var(--text-muted); background:var(--surface-strong); }.move-table li:nth-child(even) { background:color-mix(in srgb,var(--surface-strong) 45%,transparent); }.move-table li > span { color:var(--text-muted); }.move-table button { color:var(--text); }.move-table button:hover,.move-table button.active { color:var(--text); background:var(--surface-hover); }.move-table button.active { box-shadow:inset 3px 0 var(--accent); }.move-table button:focus-visible { outline-color:var(--accent); }
</style>
