...
<template>
  <article class="live-card broadcast-card">
    <NuxtLink class="card-link" :to="`/watch/broadcast/${encodeURIComponent(game.game_id)}`" :aria-label="`Assistir ${game.white.name} contra ${game.black.name}`">
      <header><span class="live-badge">● AO VIVO</span><span>{{ game.tournament }} · {{ game.round }}</span></header>
      <div class="players">
...
header { display: grid; gap: .35rem; min-width: 0; } header > span:last-child { overflow: hidden; font-size: .76rem; font-weight: 700; text-overflow: ellipsis; white-space: nowrap; transition: color 160ms ease; }
.live-badge { width: max-content; padding: .25rem .45rem; color: var(--danger); background: var(--danger-soft); border-radius: 999px; font-size: .65rem; font-weight: 800; letter-spacing: .06em; }
