...
<style scoped>
.bots-page { min-height: 100vh; padding: clamp(1rem, 2.5vw, 2rem); color: #3c2b20; background-color: #f4eddf; background-image: radial-gradient(#bba98e35 .7px, transparent .7px); background-size: 5px 5px; font-family: Inter, system-ui, sans-serif; }
header { max-width: 760px; margin-bottom: 1.15rem; } header > span { color: #6f4528; font-size: .65rem; font-weight: 800; letter-spacing: .14em; } h1 { margin: .18rem 0; font: 500 clamp(2rem, 4vw, 3.15rem)/1 Georgia, serif; } header p { margin: .35rem 0 0; color: #806d5d; line-height: 1.45; }
.bot-grid { display: grid; grid-template-columns: repeat(5, minmax(140px,1fr)); gap: .8rem; max-width: 1120px; }.bot-card { --level: #6f4528; display: grid; min-height: 390px; justify-items: center; align-content: start; gap: .45rem; padding: 1rem .8rem 1.15rem; color: inherit; background: linear-gradient(180deg,#fffaf0f2,#fbf3e6e8); border: 1px solid #e1d1ba; border-radius: 18px; box-shadow: 0 10px 24px #6f452810; cursor: pointer; transition: transform 160ms, border-color 160ms, box-shadow 160ms; }.bot-card:hover { transform: translateY(-3px); border-color: #c0a37b; box-shadow: 0 15px 30px #6f452817; }.bot-card.selected { border: 2px solid var(--level); box-shadow: 0 12px 28px color-mix(in srgb,var(--level) 25%,transparent); }.bot-card:focus-visible { outline: 3px solid #d0a45d; outline-offset: 3px; }.portrait-wrap { position: relative; display: block; width: min(100%,176px); aspect-ratio: 1; margin-bottom: .35rem; }.portrait-wrap img { width: 100%; height: 100%; object-fit: cover; border: 3px solid #ead4b3; border-radius: 50%; box-shadow: 0 6px 18px #6f452820; }.portrait-wrap > i { position: absolute; top: 0; right: 0; display: grid; width: 30px; height: 30px; place-items: center; color: white; background: var(--level); border: 3px solid #fff8ec; border-radius: 50%; font-style: normal; font-weight: 900; }.difficulty { padding: .25rem .65rem; color: var(--level); background: color-mix(in srgb,var(--level) 15%,#fffaf0); border-radius: 999px; font-size: .66rem; font-weight: 900; text-transform: uppercase; letter-spacing: .07em; }.bot-card strong { margin-top: .1rem; font: 600 1.55rem Georgia, serif; }.bot-card small { min-height: 2.8rem; color: #766252; line-height: 1.45; }.bot-card b { margin-top: auto; color: var(--level); font-size: 1.35rem; }.strength { display: inline-flex; height: 18px; align-items: end; gap: 2px; }.strength i { display: block; width: 4px; height: 7px; background: currentColor; }.strength i:nth-child(2) { height: 12px; }.strength i:nth-child(3) { height: 17px; }.bot-clark { --level: #3f6a42; }.bot-jonathan { --level: #365f85; }.bot-renan { --level: #8b5616; }.bot-boris { --level: #5a3e78; }.bot-terminator { --level: #8b2d25; }
.setup-card { display: grid; max-width: 1120px; grid-template-columns: minmax(230px,.75fr) minmax(420px,1.5fr) minmax(250px,.85fr); align-items: stretch; margin-top: 1.2rem; overflow: hidden; background: #fffaf0e8; border: 1px solid #dfcfb8; border-radius: 18px; box-shadow: 0 14px 34px #6f452812; }.selected-rival { display: flex; align-items: center; gap: .8rem; padding: 1rem 1.15rem; background: #f5e9d7; border-right: 1px solid #dbc6a8; }.selected-portrait { width: 62px; height: 62px; flex: 0 0 auto; object-fit: cover; border: 2px solid #d7ba92; border-radius: 50%; }.selected-rival > div { display: grid; gap: .08rem; }.selected-rival span { color: #6f4528; font-size: .62rem; font-weight: 900; letter-spacing: .11em; }.selected-rival strong { font: 600 1.3rem Georgia,serif; }.selected-rival small { color: #806d5d; }
.match-options { display: grid; grid-template-columns: minmax(150px,.75fr) minmax(245px,1.25fr); align-items: end; gap: 1rem; padding: .9rem 1.1rem; }.time-option { display: grid; gap: .4rem; }.setup-card label > span, legend { color: #806d5d; font-size: .72rem; font-weight: 800; }select { width: 100%; padding: .7rem .8rem; color: inherit; background: #fffaf0; border: 1px solid #d8c4aa; border-radius: 10px; font: inherit; }fieldset { display: flex; gap: .4rem; margin: 0; padding: 0; border: 0; }legend { margin-bottom: .4rem; }fieldset label { position: relative; flex: 1; }fieldset input { position: absolute; opacity: 0; }fieldset span { display: block; padding: .68rem .55rem; text-align: center; white-space: nowrap; background: #f3e7d5; border: 1px solid #dfcfb8; border-radius: 9px; cursor: pointer; transition: color 140ms, background 140ms, border-color 140ms; }fieldset input:checked + span { color:#4b3829; background:#eadcc7; border-color:#c9a976; box-shadow:inset 0 0 0 1px #fff8ec,0 5px 12px #6f452812; }fieldset input:focus-visible + span { outline: 3px solid #d0a45d; outline-offset: 2px; }.color-option { display:flex; align-items:center; justify-content:center; gap:.42rem; }.pawn-icon { width:20px; height:20px; flex:0 0 auto; overflow:visible; stroke-width:1.35; stroke-linejoin:round; }.white-option .pawn-icon { fill:#fffdf8; stroke:#352a23; filter:drop-shadow(0 1px 1px #352a2340); }.black-option .pawn-icon { fill:#25201c; stroke:#fff8ec; }.random-option i { color:#9a6a2b; font:800 1.15rem/1 Inter,system-ui,sans-serif; }
.play { display: grid; align-content: center; gap: .2rem; padding: .8rem 1.15rem; color: white; text-align: left; background: #6f4528; border: 0; border-left: 1px solid #5a351f; cursor: pointer; transition: background 150ms ease, transform 150ms ease, box-shadow 150ms ease; box-shadow: inset 0 1px #fff3d414, 0 12px 22px #6f452824; }.play:hover { background: #7f5130; transform: translateY(-1px); }.play > span { font: 700 1.08rem Georgia,serif; }.play small { color: #f1dfcf; }.play b { float: right; margin-left: .5rem; font-size: 1rem; }.play:disabled { opacity: .6; cursor: wait; }.setup-card > .error { grid-column: 1/-1; margin: 0; padding: .65rem 1rem; background: #f6ded5; }.error { color: #a44232; }.state { padding: 2rem; text-align: center; }
