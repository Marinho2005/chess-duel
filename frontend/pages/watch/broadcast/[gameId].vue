...
import type { BroadcastLiveGame, BroadcastClockSnapshot, LiveMove } from '~/types/live-games'
...
import { broadcastClocks, formatBroadcastClock, createBroadcastClockEstimate } from '~/utils/broadcastClocks'
...
const clockEstimate = createBroadcastClockEstimate()
const broadcastEnded = ref(false)

const replayPositions = computed(() => buildReplayPositions(game.value?.moves || [], game.value?.initial_fen || undefined))
const displayedPly = computed(() => viewedPly.value ?? livePly.value)
...
const displayedFen = computed(() => (viewingHistory.value || livePly.value < (game.value?.moves.length ?? 0))
  ? displayedPosition.value?.fen || game.value?.fen || ''
  : game.value?.fen || '')
const displayedLastMove = computed<[Key, Key] | null>(() => {
  if (viewingHistory.value || livePly.value < (game.value?.moves.length ?? 0)) {
...
const recordedClocks = computed(() => {
  const recorded = broadcastClocks(game.value?.moves || [], displayedPly.value, game.value?.initial_fen)
  const live = game.value?.live_clock
  return !viewingHistory.value && displayedPly.value === game.value?.moves.length && live
    ? { white: live.white_ms ?? recorded.white, black: live.black_ms ?? recorded.black }
    : recorded
})
const activeClock = computed(() => completed.value || broadcastEnded.value || state.value === 'ended'
  ? null
  : displayedFen.value.split(' ')[1] === 'b' ? 'black' : 'white')
const estimatingClock = computed(() => !viewingHistory.value
  && displayedPly.value === game.value?.moves.length && activeClock.value !== null)
const displayedClocks = computed(() => ({
  white: clockEstimate.remaining(recordedClocks.value.white, estimatingClock.value && activeClock.value === 'white', clockNow.value),
  black: clockEstimate.remaining(recordedClocks.value.black, estimatingClock.value && activeClock.value === 'black', clockNow.value),
}))
watch(game, (snapshot) => {
  if (!snapshot) return
  const clocks = broadcastClocks(snapshot.moves, snapshot.moves.length, snapshot.initial_fen)
  const now = performance.now()
  clockEstimate.sync(`${snapshot.game_id}:${snapshot.moves.length}:${snapshot.fen}`, snapshot.live_clock?.white_ms ?? clocks.white, snapshot.live_clock?.black_ms ?? clocks.black, now, snapshot.live_clock)
  clockNow.value = now
}, { flush: 'sync' })

function clockTitle(color: 'white' | 'black') {
  if (displayedClocks.value[color] === null) return 'Tempo não informado pela transmissão'
  return estimatingClock.value && activeClock.value === color
    ? 'Tempo estimado, sincronizado com os dados da transmissão'
    : 'Último tempo registrado até este lance pela transmissão'
}
...
onMounted(async () => {
  ...
  clockTimer = setInterval(() => { clockNow.value = performance.now() }, 250)
  auth.restoreSession()
  ...
  channel.on('broadcast_ended', () => {
    broadcastEnded.value = true
    state.value = 'ended'
    if (roundId.value) void loadRoundGame()
  })
  channel.join()
    .receive('ok', (snapshot: BroadcastLiveGame) => {
      if (staleSnapshot(snapshot)) return
      broadcastEnded.value = false
      game.value = snapshot
      state.value = 'loaded'
...
function staleSnapshot(snapshot: { moves: LiveMove[]; live_clock?: BroadcastClockSnapshot | null }) {
  if (!game.value || snapshot.moves.length > game.value.moves.length) return false
  const currentSample = game.value.live_clock?.sampled_at_ms
  const incomingSample = snapshot.live_clock?.sampled_at_ms
  return currentSample != null && incomingSample != null && incomingSample < currentSample
}
...
function applyMove(payload: { fen: string; last_move: LiveMove | null; moves: LiveMove[]; live_clock?: BroadcastClockSnapshot | null }) {
  if (!game.value || completed.value || staleSnapshot(payload)) return
  broadcastEnded.value = false
  game.value = { ...game.value, fen: payload.fen, last_move: payload.last_move, moves: payload.moves, live_clock: payload.live_clock }
  state.value = 'loaded'
...
function playNextMove() {
  if (!game.value || livePly.value >= game.value.moves.length) return
  livePly.value += 1
  const move = game.value.moves[livePly.value - 1]
  if (move) sounds.play(soundForSan(move.san))
...
// HTTP and channel updates share playback, so batches preserve each intermediate position.
watch(game, (snapshot, previous) => {
  if (!snapshot) return
  const extendsHistory = previous && snapshot.initial_fen === previous.initial_fen
    && snapshot.moves.length >= previous.moves.length
    && previous.moves.every((move, index) => {
      const next = snapshot.moves[index]
      return next?.from === move.from && next.to === move.to && next.promotion === move.promotion
    })
  if (!extendsHistory || viewingHistory.value) {
    stopPlayback()
    livePly.value = snapshot.moves.length
    if (viewedPly.value !== null) viewedPly.value = Math.min(viewedPly.value, snapshot.moves.length)
...
function selectPly(ply: number) {
  ...
  stopPlayback()
  livePly.value = game.value.moves.length
    ...
    <section v-else class="watch-layout">
      <div class="board-column">
        <article class="player-bar">
          ...
          <span class="broadcast-clock" :class="{ active: activeClock === 'black', unavailable: displayedClocks.black === null }" :title="clockTitle('black')" :aria-label="`Tempo das pretas: ${formatBroadcastClock(displayedClocks.black)}`">{{ formatBroadcastClock(displayedClocks.black) }}</span>
        </article>
        <div class="board-with-evaluation">
          ...
          <div class="board-frame">
            <GameReadonlyBoard :animation-duration="moveDuration" :fen="displayedFen" :last-move="displayedLastMove" :label="`Broadcast de ${game.white.name} contra ${game.black.name}`" />
          </div>
        </div>
        <article class="player-bar">
          <span class="piece" aria-hidden="true">♙</span>
          ...
          <span class="broadcast-clock" :class="{ active: activeClock === 'white', unavailable: displayedClocks.white === null }" :title="clockTitle('white')" :aria-label="`Tempo das brancas: ${formatBroadcastClock(displayedClocks.white)}`">{{ formatBroadcastClock(displayedClocks.white) }}</span>
        </article>
      </div>
...
<style scoped>
.broadcast-clock { min-width: 6.5ch; padding: .5rem .65rem; border: 1px solid var(--border); border-radius: 8px; background: var(--surface-strong); color: var(--text-muted); font: 700 clamp(1.05rem, 2.2vw, 1.6rem)/1.2 var(--font-mono); font-variant-numeric: tabular-nums; text-align: center; white-space: nowrap; }
.broadcast-clock.active { color: var(--accent-ink); background: var(--accent); border-color: var(--accent); }
.broadcast-clock.unavailable { color: var(--text-muted); background: var(--surface-strong); border-color: var(--border); }
...
.watch-page { min-height:calc(100vh - 60px); padding:0 clamp(1rem,3vw,2.5rem) 1.5rem; color:var(--text); background:var(--bg); font-family:var(--font-sans); }
...
.player-bar { display:grid; grid-template-columns:auto minmax(0,1fr) auto; align-items:center; gap:.8rem; min-height:66px; padding:.65rem .85rem; background:color-mix(in srgb,var(--surface) 94%,transparent); border:1px solid var(--border-subtle); border-radius:13px; }.player-bar .piece { display:grid; width:44px; height:44px; place-items:center; color:var(--accent-ink); background:var(--accent); border-radius:50%; font-size:1.7rem; }.player-bar div { display:grid; min-width:0; gap:.12rem; }.player-bar strong { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }.player-bar small { color:var(--text-muted); }
.game-panel { display:flex; min-height:min(760px,calc(100vh - 85px)); flex-direction:column; overflow:hidden; background:color-mix(in srgb,var(--surface) 96%,transparent); border:1px solid var(--border); border-radius:18px; box-shadow:var(--shadow); }.panel-heading { padding:1.5rem; border-bottom:1px solid var(--border); }.panel-heading h1 { margin:.55rem 0 .3rem; font:500 1.55rem var(--font-serif); }.panel-heading>p { margin:0; color:var(--text-muted); }.live-badge { display:inline-block; padding:.3rem .5rem; color:var(--danger); background:var(--danger-soft); border-radius:999px; font-size:.68rem; font-weight:800; letter-spacing:.05em; }
.suggested-line { display:grid; gap:.32rem; margin-top:1rem; padding:.72rem .8rem; background:var(--surface-strong); border:1px solid var(--border-subtle); border-radius:9px; }.suggested-line strong { color:var(--text); font-size:.72rem; letter-spacing:.04em; text-transform:uppercase; }.suggested-line code,.suggested-line span { color:var(--text-muted); font:500 .82rem/1.45 var(--font-mono); overflow-wrap:anywhere; }.suggested-line code { color:var(--text); }
.moves { display:flex; min-height:0; flex:1; flex-direction:column; gap:.85rem; padding:1.2rem 1.5rem; }.moves-title { display:flex; align-items:center; justify-content:space-between; gap:.6rem; }.moves-title h2 { margin:0; font:500 1.25rem var(--font-serif); }.moves-title>div { display:flex; align-items:center; gap:.3rem; }.moves-title button { min-width:28px; height:28px; padding:0 .45rem; color:var(--accent); background:var(--surface-strong); border:1px solid var(--border); border-radius:7px; font-size:.7rem; font-weight:800; cursor:pointer; }.moves-title button:disabled { opacity:.4; cursor:default; }.moves-title span { display:grid; min-width:42px; height:28px; place-items:center; color:var(--text-muted); background:var(--surface-strong); border-radius:999px; font-size:.7rem; }.empty-moves { margin:auto; color:var(--text-muted); text-align:center; }.keyboard-hint { color:var(--text-muted); font-size:.7rem; text-align:center; }.lichess-link { display:flex; align-items:center; justify-content:center; gap:.4rem; padding:1rem; color:var(--accent); border-top:1px solid var(--border); font-weight:700; text-decoration:none; }.lichess-link:hover { background:var(--surface-hover); }
