...
async function handleMove(move: { from: Key; to: Key; promotion?: 'q' }) {
  ...
  try { chess.move({ from: move.from as Square, to: move.to as Square, promotion: move.promotion }) } catch { return }
...
function applyAutomaticMove(uci: string) { if (!chess) return; const move = parseUci(uci); chess.move(move); boardFen.value = chess.fen(); lastMove.value = [move.from as Key, move.to as Key]; updateBoard() }
...
<template>
  <main class="battle-page" :class="`layout-${boardLayoutSize}`">
    <header><div><NuxtLink to="/puzzles"><ArrowLeft :size="16" /> Todos os modos</NuxtLink><span>BATALHA DE PROBLEMAS · {{ duration / 60 }} MIN</span><h1>Duelo tático</h1></div><fieldset><legend>Tabuleiro</legend><button v-for="option in [{value:'compact',label:'P'},{value:'standard',label:'M'},{value:'large',label:'G'}]" :key="option.value" :class="{active:boardLayoutSize===option.value}" @click="setBoardLayoutSize(option.value as BoardLayoutSize)">{{ option.label }}</button></fieldset></header>

    <section v-if="phase === 'connecting' || phase === 'searching' || phase === 'error'" class="waiting"><span><Search v-if="phase !== 'error'" :size="30" /><X v-else :size="30" /></span><h2>{{ phase === 'searching' ? 'Buscando oponente' : phase === 'error' ? 'Algo deu errado' : 'Conectando' }}</h2><p>{{ message }}</p><NuxtLink v-if="phase === 'error'" to="/puzzles">Voltar aos modos</NuxtLink></section>

    <section v-else class="arena">
      <div class="board-wrap"><div v-if="phase === 'preparing'" class="countdown">{{ Math.max(1, Math.ceil(countdownMs / 1000)) }}</div><GameBoard v-if="puzzle" :fen="boardFen" :orientation="orientation" :turn-color="turnColor" :last-move="lastMove" :check="inCheck" :disabled="boardDisabled" @move="handleMove" /><div v-else class="board-empty"><Swords :size="42" /><strong>{{ phase === 'finished' ? resultTitle : 'Sequência concluída' }}</strong></div></div>
      <aside>
