...
type RushState = { session_id: string; duration_seconds: number; remaining_ms: number; score: number; errors: number; maximum_errors: number; finished: boolean; puzzle?: Puzzle }
...
const boardDisabled = computed(() => loading.value || submitting.value || !puzzle.value || ['solved', 'failed', 'expired'].includes(feedback.value))
...
async function handleMove(move: { from: Key; to: Key; promotion?: 'q' }) {
  ...
  try {
    chess.move({ from: move.from as Square, to: move.to as Square, promotion: move.promotion })
  } catch {
...
function applyRushState(state: RushState) {
...
<template>
  <main class="puzzles-page" :class="`layout-${boardLayoutSize}`">
    <header class="page-header">
      <div>
        <h1>{{ mode === 'classic' ? 'Modo Clássico' : 'Corrida de problemas' }}</h1>
        <p>Encontre a melhor sequência. O adversário responde automaticamente a cada acerto.</p>
      </div>
    ...
    <section v-if="mode === 'rush' && !rushSessionId" class="rush-setup">
      ...
      <fieldset><legend>Duração</legend><label><input v-model="rushDuration" type="radio" :value="180"><span>3 minutos</span></label><label><input v-model="rushDuration" type="radio" :value="300"><span>5 minutos</span></label></fieldset>
      <button class="primary-action" :disabled="loading" @click="startRush">{{ loading ? 'Preparando…' : 'Começar o Rush' }}</button>
...
.play-layout { display:grid; width:fit-content; max-width:100%; grid-template-columns:var(--board-limit) minmax(270px,340px); align-items:start; justify-content:center; gap:clamp(1rem,2vw,1.6rem); margin:-.5rem auto 0; }.board-column { width:var(--board-limit); max-width:100%; justify-self:end; }.board-placeholder { display:grid; width:100%; aspect-ratio:1; place-items:center; color:#7e6957; background:#e9dcc8; border:1px solid #d3bea0; border-radius:10px; }.board-placeholder.error { color:#9c3f34; }
